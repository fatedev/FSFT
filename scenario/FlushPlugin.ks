...
//		var fl = flushlayer = win.fore.layers[useLayerStart];
		...
		if(elm.layer === void)
		{
			...
			if(i == ls.count)
			{
				win.allocateCharacterLayersWithoutOrder(ls.count + 1);
				fl	= flushlayer	= ls[i];
			}
			_layer	= i;
		}
		else
			fl	= flushlayer	= ls[elm.layer];
		with(fl)
		{
			.setPos(0, 0);
			.opacity	= 0;
...
//		dm("flush opacity("+flushlayer.visible+") = "+flushlayer.opacity);
		flushlayer.visible		= _opacity > 0 ? true : false;
	}
...
@eval exp="mp.storage=flush_object._storage"
@eval exp="mp.color=flush_object._color"
@eval exp="mp.layer=flush_object._layer"
