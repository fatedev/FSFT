...
//			dm(@"flow[current] = ${.type}");
			...
			case "SELECTER":
				//	選択肢を表示する
				var handlers	= window.tagHandlers;
				var elm = %[];
				elm["text"]		= "";
...
/*
					...
					handlers.r();	//	次行へ
*/
					//	AlternativePlugin を使用
					var	alt	= .alternative[i];
					handlers.addalter(%[
						text:alt.text,
						label:"*play",
						exp:"global.flow_tracker_object.current="+alt.target,
						enabled:true,
						align:"center"
					]);
				}
...
with(kag)
{
	.debugMenu.add(kag.flowchartMenuItem = new KAGMenuItem(this, "フローチャート表示(&F)", 1, function(sender) { global.flow_tracker_object.isTest = !global.flow_tracker_object.isTest; sender.checked = global.flow_tracker_object.isTest; }, false));
...
@bg cond="kag.fore.base.storage!=skbg" file=&skbg time=600 method=crossfade
...
@eval exp='kag.process(,askYesNo("　\n"+.date+"\n　　"+.title+"\n\n　このシーンをスキップしますか？\n　")?"*skip_to_next":"*sceneplay");'
@s
@endif
;シーンを再生する
*sceneplay
