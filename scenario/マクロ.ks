...
;moveタグの拡張
@iscript
kag.tagHandlers.moveorg	= kag.tagHandlers.move;
kag.tagHandlers.move	= function(mp)
{
	if(mp.layer == "")
	{
		//	pos => layer no.
		if(mp.pos != "")
			mp.layer	= string(f.layer_no[mp.pos = f.curtailed_word[mp.pos]]);
		else
			throw new Exception("moveするには、layerかposを指定する必要があります。");
	}
	return kag.tagHandlers.moveorg(mp);
} incontextof kag;
@endscript
...
;表示レイヤーを表示位置から指定する
@eval cond="mp.layer==''" exp="mp.layer=string(f.layer_no[mp.pos=f.curtailed_word[mp.pos]])"
;表示する時は必ず255にする
...
@if exp="mp.pos!='all'"
@eval cond="mp.layer==''" exp="mp.layer=string(f.layer_no[f.curtailed_word[mp.pos]])"
...
@eval exp="mp.file=mp.storage if mp.file==''"
...
;------------------------------------------------------------------------------
@iscript
kag.tagHandlers.fadeseorg= kag.tagHandlers.fadese;
kag.tagHandlers.fadese	= function(mp)
{
	var buf;
	if(mp.buf !== void)
		buf	= +mp.buf;
	else if(mp.file != void)
		buf	= getPlayBuffer(mp.file);
...
@graph storage=&('line'+kag.current.fontSize+'-'+mp.len) alt=&.alt
@endmacro
...
;FUNC--------------------------------------------------------------------------
;	タイガー道場のキャラクターべしゃり(マキジ)
;	in	: 
;------------------------------------------------------------------------------
@macro name=talkMakiji
@position page=fore frame='tw_カエデ'
@position page=back frame='tw_カエデ'
@ct
@endmacro
...
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
;[macro name=say][endmacro]
[macro name=sethollowmode][eval exp=f.gamemode='hollow'][cm][eval exp="popupMessage('ここから hollow モードに入ります。Fate_PS2環境では例外が発生する恐れがあります。')"][endmacro]
[macro name=setfatemode][eval exp=f.gamemode=''][eval exp="popupMessage('ここから fate モードに入ります。')"][endmacro]
[macro name=nolr][endmacro]
...
@eval exp='kag.isClickRepeat=false'
@er
@return
