	...
	function changeDate(per)
	{
		...
		if(mStart>=0)
		{
			...
			with(monthLayer[0])
			{
				...
				monthBase.stretchBlend(0, 0, w, h, monthLayer[0], 0, 0, w, .imageHeight, 255 - opa, stCubic);
				t = h;
				h = .imageHeight - h;
			}
			var ml = monthLayer[1];
			monthBase.stretchBlend(0, t, w, h, ml, 0, 0, w, ml.imageHeight, opa, stCubic);
		}
		...
		with(dayLayer[0])
		{
			...
			dayBase.stretchBlend(0, 0, w, h, dayLayer[0], 0, 0, w, .imageHeight, 255 - opa, stCubic);
			t = h;
			h = .imageHeight - h;
		}
		var dl = dayLayer[1];
		dayBase.stretchBlend(0, t, w, h, dl, 0, 0, w, dl.imageHeight, opa, stCubic);
	}
