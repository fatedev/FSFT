...
class HazePlugin extends KAGPlugin
{
	...
	function finalize()
	{
		stop();
		release(layer);
		for(var i=0; i<tmplayers.count; i++)
			release(tmplayers[i]);
		super.finalize(...);
	}

	property tmplayer	{ getter { return tmplayers[lphase]; } }
	...
	function initLayer(page, layer, lp=lphase)
	{
		//	画像を保存する画像を用意
		var	oldimage	= true;
		var	tmp	= tmplayers[lp];
		if(tmp==void)
		{
			with(tmp = tmplayers[lp] = new global.Layer(window, window.primaryLayer))
				.name	= "Haze用テンポラリ";
			oldimage	= false;
		}
//		else
...
//				dm("lwaves["+i+"][2] = "+lwaves[i][2]);
		...
		//	元々tmpが存在している場合、tmpを作り直す必要はなし
//		dm(@"oldimage = ${oldimage} / trg = (${trg.width}, ${trg.height}) / tmp = (${tmp.width}, ${tmp.height}) => ${oldimage && trg.width == tmp.width && trg.height == tmp.height}");
		if(oldimage)
		{
			//	ターゲットのレイヤーとtmpのサイズが異なる場合
			if(trg.width != tmp.width || trg.height != tmp.height)
			{
				//	ターゲットのレイヤーサイズを調整する
...
//				dm(@"mw = ${mw}, mh = ${mh}");
				...
				tmp.setImageSize(.width + (mw<<1), .height + (mh<<1));
				tmp.setSizeToImageSize();
				tmp.copyRect(mw, mh, trg, 0, 0, .width, .height);
				with(tmp)
				{
			...
			else
			{
				...
				tmp.setSizeToImageSize();
				tmp.copyRect(0, 0, trg, 0, 0, .width, .height);
			}
//			dm(@"(${.width}, ${.height}) (${tmp.width}, ${tmp.height})");
		}
	}
...
*/
		...
		//	トランジション中のみ
//		dm("construct: inTransition = "+inTransition, "noExchgTmpLayer = "+noExchgTmpLayer);
		if(inTransition)
		{
			//	同じパラメータで裏も描画する
			var	trg;
			...
			if(noExchgTmpLayer)
			{
				...
				dohaze(trg, tick, per, (lphase+1)&1);	//	
		}
	}
...
//		dm(@"tick: ${tick} / per: ${per} / phase: ${phase}");
	...
	/	実際に揺れ処理を行う
	/------------------------------------------------------------------------*/
	function dohaze(dest, tick, per, lp=lphase)
	{
		//	トランジション中でなく、対象が表示されていなかったら、
		if(!inTransition && !dest.visible)
			return;	//	揺れ処理を行わない

		var	tmp	= tmplayers[lp];
		if(tmp == void)
			return;
//		dm("dohaze("+dest, tick, per, lp+")");
		//	保存しておいたレイヤーとサイズが異なる場合は、
		if(dest.width != tmp.width || dest.height != tmp.height)
			adjustTargetLayer(dest, true);	//	サイズを調整する

		try
		{
//			dm(@"destination: ${dest} (${dest.left}, ${dest.top})-(${dest.width}, ${dest.height})");
//			dm(@"source:      ${tmp} (${tmp.width}, ${tmp.height})");
			doHaze(%[dest:dest, src:tmp, tick:tick, per:per,
...
//			.f.derangedScreen	= false;	//	画面が乱れていない状態(セーブ可)
			...
			for(var i=0; i<tmplayers.count; i++)
			{
//				tmplayers[i].setImageSize(32, 32) if tmplayers[i] != void;
				if(tmplayers[i] != void)
				{
					invalidate tmplayers[i];
					tmplayers[i]	= void;
				}
			}
			endHaze();
...
@eval exp=haze_object.initBeforeTransition()
@endif
@endmacro
;----
;@haze_back
...
;トランジションを超えて揺らす
@eval cond=haze_object.doing exp=haze_object.initBeforeTransition()
@transex * time=%time|0
