...
class ShortcutBaseLayer extends FadeLayer
{
	...
	function hide(time=lasttime, accel=lastaccel)
	{
		if(showed)
		{
			if(extend instanceof "SaveLoadMenuLayer")
				extend.releaseSavedata();
			if(typeof extend.showed != "undefined")
				extend.showed	= false;	//	閉じている途中であるとする
			showed	= false;
			...
			removeMode();				//	モーダルOFF
			kag.leftClickHook.erase(0);	//	左クリックを元に戻す
		}
	}
...
//		dm("on key down: "+key);
		...
		{
			if(key == VK_F4 || key >= VK_F10)
				popupMessage("押下されたファンクションキーには、機能が割り当てられていません。");
			else
			{
				if(shift & ssAlt)
				{
	...
	{
		var	scnfn	= window.getBookMarkScenario(kag.numBookMarks - 1 - no);
		if(scnfn == "")
			popupMessage("[F"+(no+1)+"]にはセーブデータが存在しません。");
		else if(Storages.isExistentStorage(scnfn))
