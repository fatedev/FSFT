...
//			dm(@"place = ${place} / condition = ${condition[target]}");
	...
	function resetCondition(mp)
	{
		var changed	= false;

		if(mp.target!="fg")
			changed	|= innerResetCondition(mp, 0);
		if(mp.target!="bg")
			changed	|= innerResetCondition(mp, 1);

		return changed;
	}

	function innerResetCondition(mp, target)
	{
		if(condition[target] != "none")
		{
			condition[target]	= "none";
			current_background	= "";
			return true;
...
//		dm("ConditionPlugin.onRestore");
		...
		if(bl > 0)
		{
			...
			for(var i=0; i<pages.count; i++)
			{
				...
				if(br == "all" || br == page)
				{
					with(kag[page].base)	.doBoxBlur(bl, bl), .doBoxBlur(1, 1);
					for(var i=0; i<kag[page].layers.count; i++) with(kag[page].layers[i])
					{
						if(!.visible)
							continue;
						layer.setSize(.width, .height);
						layer.copyRect(0, 0, kag[page].layers[i], 0, 0, .width, .height);
						...
						.doBoxBlur(bl, bl);
						.doBoxBlur(1, 1);	//	boxblur 特有の模様(?)を隠すため、もう一度行う
//						dm(.left, .top, .Anim_loadParams.corr_left, .Anim_loadParams.corr_top);
					}
				}
			}
			invalidate layer;
...
kag.tagHandlers.imageex	= function(mp)
{
	...
	.sf.displayedPictures[mp.storage.toLowerCase()]++;//	既表示フラグを立てる
...
//	dm("isbg: "+mp.isbg);
	...
	if(mp.absolute !== void)
		layer.absolute	= +mp.absolute;
	else if(mp.index !== void)
		layer.absolute	= +mp.index;
	else if(+mp.layer == mp.layer)
		layer.absolute	= (+mp.layer + 1) * 1000;
...
//	dm("mp.grayscale: "+mp.grayscale);
	...
	//	画像読み込み(ぼかし)
	var bl = .f.blur_level, br = .f.blur_range;
	dm(mp.storage+": "+bl, br, bl > 0, !bluroff, br == "all", mp.isbg && (br == "base"|| br == "back"), !mp.isbg && br == "fore", br == mp.layer);
...
//			var layers = kag.fore.layers;	//	noclear = true: 表に表示されている前景レイヤーを裏に読み込みなおす
			...
			{
				var layer	= layers[i];
//				dm("kag."+pages[i]+".layers["+i+"].visible = "+layer.visible);
				if(layer.visible && layer.Anim_loadParams != void)
				{
					var elm = %[];
...
kag.tagHandlers.update_screen = function(mp)
{
	...
	//	○	foreに指定が無ければbackはそのまま	-> Fateモード時
	var th = tagHandlers;
...
//- 後処理メソッドをクリアする ------------------------------------------------
kag.tagHandlers.clearpostoperate = function(mp)
{
	condition_object.postOperate.clear() if condition_object.postOperate != void;
	cinesco(false);
	return 0;
} incontextof kag;
...
//- シネスコモードのON/OFF ----------------------------------------------------
kag.tagHandlers.cinesco = function(mp)
{
/*	global.addPostOperate(condition_object.drawCinescoBar);
	tagHandlers.imageex(%[page:"back", layer:"base", storage:back.base.storage, isbg:1]);	//	再描画
	f.cinesco	= true;
*/	cinesco(true);
	return 0;
} incontextof kag;
kag.tagHandlers.cinesco_off = function(mp)
{
/*	global.erasePostOperate(condition_object.drawCinescoBar);
	tagHandlers.imageex(%[page:"back", layer:"base", storage:back.base.storage, isbg:1]);	//	再描画
	delete f.cinesco;
*/	cinesco(false);
	return 0;
} incontextof kag;
kag.tagHandlers.iscinesco = function(mp)
{
//	return global.isAddedPostOperate(condition_object.drawCinescoBar);
	return f.cinesco;
} incontextof kag;
...
@macro name=blur_screen
@eval exp=.st=System.getTickCount()
@if exp=f.blur_range=='all'&&f.blur_level>0
@eval exp="for(var i=0;i<kag.back.layers.count;i++)with(kag.back.layers[i]){if(.visible){kag.back.base.pileRect(.left,.top,layer,0,0,.imageWidth,.imageHeight,.opacity,false);.visible=false;}}with(kag.back.base){var bl=f.blur_level;bl=bl\3 if(f.gamemode=='');.doBoxBlur(bl,bl);.doBoxBlur(1,1);}"
@endif
...
;コンディションをリセット
@macro name=resetcondition
@eval exp=condition_object.resetCondition(mp)
@endmacro
