...
@macro name=dash
;コンディションをリセット
;	dash用の画像が指定されていて、その画像が背景で無い場合
@resetcondition cond="mp.storage!='' && mp.storage[0]!='i' && mp.storage[0]!='o'"
...
@else
@eval cond="mp.storage!=void" exp=".sf.displayedPictures=%[]if(.sf.displayedPictures==void);.sf.displayedPictures[mp.storage.toLowerCase()]++"
@endif
...
;画面が乱れた状態になる
@eval exp="f.derangedScreen=true,f.prepareArrange=false"
@endmacro
...
;コンディションをリセット
;	splinemove用の画像が指定されていて、その画像が背景で無い場合
@resetcondition cond="mp.storage!='' && mp.storage[0]!='i' && mp.storage[0]!='o'"
;@resetcondition
;基本的にテキスト消去する("textoff == false"なら、消去しない)
@textoff cond="mp.textoff===void || mp.textoff"
