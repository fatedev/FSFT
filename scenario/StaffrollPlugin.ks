	...
	function addText(opt)
	{
		...
		if(!vertical)
		{
			if(opt.x !== void)
				...
				x = (kag.primaryLayer.width-fore.width) \ 2 + (opt.cmx!=void ? +opt.cmx : 0);
			x += baseline;
			y = currentPos += +opt.y;
		}
		else
...
//			.visible= false;
	...
	function startMove(height, time)
	{
		...
		{
			if(height[0] == "+")
				height	= Math.abs(currentPos) + int(height.substr(1));
			else if(height[0] == "-")
				height	= Math.abs(currentPos) - int(height.substr(1));
			else
				height	= int height;
...
@eval exp="staffroll_object.startMove(mp.width !== void ? mp.width : mp.height, tm)"
;時間原点を設定
@resetwait
@endmacro
